...
class DAVIS_MO_Test(data.Dataset):
    ...
    def __init__(self, root, imset='2017/train.txt', resolution='480p', single_object=False):
        ...
        with open(os.path.join(_imset_f), "r") as lines:
            for line in lines:
                _video = line.rstrip('\n')
                #print(sorted(glob.glob(os.path.join(self.mask_dir, _video, '*.png'))))    
    ...
    def load_single_image_name_path(self,video,f):
        import glob
        file_name_jpg = sorted(glob.glob(os.path.join(self.mask480_dir, video,"*.png")),reverse=direction_m)[f]
